  ...
  return (
    ...
    >
      <div className={cn("", iconClassName)}>
        {/* 로고 이미지를 활용해 브랜드 컬러를 자연스럽게 노출합니다. */}
        <img src={logoImage.src} alt="서비스 로고" className="bject-fill" />
      </div>
      <motion.span
        animate={{
          display: animate ? (open ? "inline-block" : "none") : "inline-block",
        ...
        className={cn(
          "-mt-1",
          textClassName,
          open ? "inline-block" : "overflow-hidden"
        )}
      >
        {label}
      </motion.span>
...
// 사이드바가 접혔을 때 사용할 축약형 로고 컴포넌트입니다.
export const SidebarLogoIcon = ({
  href = "/",
  iconClassName,
}: Pick<SidebarLogoProps, "href" | "iconClassName">) => {
  return (
    <Link
      href={href}
    ...
    >
      <div className={cn("", iconClassName)}>
        {/* 축약형 로고에서도 동일한 이미지를 사용해 일관성을 유지합니다. */}
        <img src={logoImage.src} alt="서비스 로고" className="object-cover" />
      </div>
    </Link>
  );
};
